...
var ConnectionPool = function(config){
     ...
     * @return 
     */
    self.on('free', function(connction){
	...
	// 为自己辩解下: 因为是异步的connected事件,因此如果retry被置为0是不是会影响重连逻辑呢,我猜会,不过影响面应当很小,试想一个链接如果创建成功了,那么接下来的链接应该也是可以创建成功的,那么重试次数本身,没意义了应该.
	self.retry = 0;
...
/*
	...
	if(connection._events['open'].length > 0){
	    (connection._events['open']).pop();
	}
*/
	console.log(connection._events['close'].length);
